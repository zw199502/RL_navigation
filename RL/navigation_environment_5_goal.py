...
from navigation_low import Agent
...
goal_set = [[ 0.0,  0.0], [ 0.5,  0.7], [ 1.5,  0.2], [ 2.5,  0.0], [ 4.0,  0.0], 
            [ 0.7,  1.0], [ 2.0,  1.0], [ 2.8,  1.0], [ 4.2,  1.0], [ 0.0,  2.0],
            [ 1.0,  2.5], [ 2.0,  2.0], [ 3.0,  2.9], [ 4.2,  2.3], [ 4.3,  3.3],
            [ 4.3,  4.3], [ 3.1,  4.2], [ 2.0,  3.5], [ 1.6,  4.3], [ 0.4,  3.4],
            [-0.5,  0.5], [-0.5,  2.0], [-1.5,  0.5], [-2.2,  0.3], [-3.6,  0.0],
            [-4.3,  0.3], [-3.9,  1.0], [-1.7,  1.0], [-1.0,  1.8], [-0.5,  2.4],
            [-0.9,  4.2], [-2.6,  3.0], [-3.8,  2.5], [-4.2,  4.3], [-3.0,  3.8],
            [-1.0, -1.0], [-2.0, -1.0], [-3.0, -1.7], [-4.3, -0.7], [-4.2, -1.7],
            [-3.0, -2.0], [-4.0, -3.7], [-3.5, -4.0], [-2.5, -3.8], [-1.4, -4.2],
            [-2.8, -2.5], [-0.9, -3.5], [-0.7, -1.5], [ 0.6, -2.6], [ 0.7, -1.8],
            [ 1.6, -1.6], [ 0.5, -2.5], [ 0.0, -4.0], [ 1.0, -4.0], [ 2.0, -3.8],
            [ 3.2, -3.8], [ 4.0, -4.0], [ 4.2, -3.0], [ 4.0, -2.0], [ 2.5, -1.5],
            [ 4.0, -0.7], [ 1.5, -2.6], [ 1.5, -4.0], [ 2.0, -3.9], [ 4.0, -0.5]]
...
class Env:
    ...
    def getState(self, scan):
        ...
        # if rel_dis_final_goal <= self.threshold_arrive:  # version one and two
        if rel_dis_final_goal <= self.threshold_arrive or self.current_step >= 400: # version three
            arrive = True
    ...
    def step(self, action):
        ...
        linear_v = 0.0
        if action == 0:
            reward -= laser_min_range    # version one and two
            # reward -= 1.0 # version three
        if action == 1:
            linear_v = self.linear_v_current
            angular_v = self.angular_v_current

            state1_low = self.used_scan / 1.5
            state2_low = np.array([self.rel_dis / self.look_ahead, self.rel_theta / np.pi, linear_v, angular_v])

            action_low = self.agent.model.get_action(state1_low, state2_low)
        ...
        if orientation_error < -np.pi:
            orientation_error = orientation_error + 2 * np.pi
        angular_v = self.angle_to_velocity * orientation_error
        ...
        linear_v = linear_v + velocity_var[0] / 2
        if linear_v < 0:
            linear_v = 0
        ...
        if (self.min_scan_range >= laser_min_range) and (self.min_scan_range <= 1.5): # version one and two
        # if (self.min_scan_range >= self.look_ahead) and (self.min_scan_range <= 1.5) and (action == 1): # version three
        #     # reward = reward - 0.8 / self.min_scan_range * laser_min_range # version one
            reward = reward - 1.2 / self.min_scan_range * laser_min_range  # version two
        #     reward = reward - 1.0 / self.min_scan_range * laser_min_range  # version three
        # if self.min_scan_range < self.look_ahead and self.min_scan_range > 0.4: # version three
        #     reward = reward - 2.0 # version three
        # if self.min_scan_range < 0.4 and self.min_scan_range > laser_min_range: # version three
        #     reward = reward - 3.0 # version three
        ...
        if arrive:
            ...
            # reward += 10 # version one
            reward += 0.0  # version two
            self.current_step = 0
