...
# fn_state_set = 'data_processing/dqn/world3/version1/log.txt'
...
class Env:
    def __init__(self):
        ...
        self.flag_kill_threading = False
        self.thread_pub_goal = threading.Thread(target=self.pub_goal_thread)
        self.thread_pub_goal.start()  # start the treading of publishing goal
    ...
    def step(self, action):
        ...
        # velocity_var = np.random.normal(0, 0.05, 2) # version one
        # linear_v = linear_v + velocity_var[0] / 2 # version one
        velocity_var = np.clip(np.random.normal(0, 0.05, 2), -0.2, 0.2) # version two
        ...
        if (self.min_scan_range >= laser_min_range) and (self.min_scan_range <= 1.5):
            reward = reward - 0.8 / self.min_scan_range * laser_min_range
        self.linear_v_last = linear_v
        self.angular_v_last = angular_v

        if arrive:
            # print('target arrived at: ', self.current_step)
            self.flag_kill_threading = True
            vel_cmd = Twist()
            vel_cmd.linear.x = 0.
            vel_cmd.angular.z = 0.
            self.pub_cmd_vel.publish(vel_cmd)
        ...
        return state1, state2, reward, done, arrive, current_robot_pose
    ...
    def reset(self, episode):
        ...
        return state1, state2, current_robot_pose
