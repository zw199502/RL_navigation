...
class Env:
    ...
    def getState(self, scan):
        ...
        if rel_dis_final_goal <= self.threshold_arrive or self.current_step == 400:
            arrive = True
    ...
    def step(self, action):
        reward = 0.0
        linear_v = (action[0] + 1.0) / 2.0 * self.max_linear_velocity
        if linear_v <= self.max_linear_velocity / 2.0:
            reward -= ((self.max_linear_velocity - linear_v) * 2.0)
        angular_v = action[1] * self.max_angular_velocity
        # reward = reward - (self.max_linear_velocity - linear_v)
        ...
        return state, reward, done, arrive, arrive2
