...
from navigation_low_igor import Agent
...
class Env:
    ...
    def pub_goal_thread(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.min_scan_range > 0.5 or self.current_step == 0:
    ...
    def getState(self, scan):
        ...
        if rel_dis_final_goal <= self.threshold_arrive or self.current_step >= 600: # version three
            arrive = True
    ...
    def step(self, action):
        ...
        if action == 1:
            linear_v_c = self.linear_v_current
            angular_v_c = self.angular_v_current

            state1_low = self.used_scan / 1.5
            state2_low = np.array([self.rel_dis / self.look_ahead, self.rel_theta / np.pi, linear_v_c, angular_v_c])
        ...
        # add noise
        velocity_var = np.clip(np.random.normal(0, 0.05, 2), -0.2, 0.2)
        ...
        if self.current_step % (4 * self.control_frequency) == 0:
            arrive2 = True
