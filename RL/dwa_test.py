...
class Env:
    def __init__(self):
        ...
        self.initial_states = np.zeros((test_times, 5))
        # robot pose
    ...
    def step(self):
        ...
        if arrive:
            print(1, self.current_step + 1)

        if done:
            print(0, self.current_step + 1)
    ...
    def reset(self, episode):
        ...
        self.initial_states[episode, :] = np.hstack((current_robot_pose, self.goal_position))
...
if __name__ == '__main__':
    ...
    path_record = []
    # reset_time = 0 # multiple test
    reset_time = 79 # single test
    done, arrive, current_robot_pose = dwa_env.reset(reset_time)
    ...
    #     path_record.append(current_robot_pose_list)
    #     if done or arrive:
    #         fn = "data_processing/dwa/path_{}.txt".format(reset_time + 1)
    #         path_record_numpy = np.array(path_record)
    #         np.savetxt(fn, path_record_numpy)
    #         if reset_time == test_times:
    #             break
    #         path_record = []
    #         count_step = 0
